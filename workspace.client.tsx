/**
 * Workspace 面板：Rumen 的主界面。
 *
 * 六个一级入口，每个回答一个问题：
 *
 * | 入口 | 回答什么问题 |
 * |---|---|
 * | 现在 | agent 在动什么？涉不涉及我不懂的东西？ |
 * | 技术栈 | 这个项目用了什么？我懂多少？ |
 * | 学习 | 我接下来该学什么？ |
 * | 还债 | agent 写了什么我还没读？ |
 * | 提交 | 哪些代码是 agent 写的而我没读？ |
 * | 设置 | — |
 *
 * **"现在"是默认页**，因为这是唯一会实时变化、也最能体现产品价值的一屏。
 */

import { Icon, type PluginTheme, type PluginWorkspacePanelProps, useRpc, useWorkspace } from "@getpaseo/plugin";
...
type Tab = "now" | "tech" | "learn" | "review" | "commits" | "settings";
...
function NowView({ data, theme, t, onOpenTech, onGoReview }: {
  ...
  const weak = data.technologies
    .filter((item) => item.worthLearning && item.mastery.score < 60)
    .slice(0, 8);
  return (
    <View style={{ gap: 16 }}>
...
function TechView({ data, theme, t, onSelect, onClassify, classifying }: {
  data: Dashboard;
  theme: PluginTheme;
  t: Translator;
  ...
  if (data.technologies.length === 0 && data.pending.length === 0) {
    return <Empty text={t.stack_empty} theme={theme} />;
  }
  return (
    <View style={{ gap: 16 }}>
...
function WikiBox({ tech, workspaceId, cwd, clientLocale, locale, theme, t, onRead }: {
  tech: Technology;
  workspaceId: string;
  cwd: string;
  ...
  const wiki = useQuery({
    queryKey: ["rumen", "wiki", workspaceId, tech.id, locale],
    queryFn: () => getWiki({ workspaceId, cwd, clientLocale, techId: tech.id, lang: locale }),
    retry: 0,
  });
  const generating = useMutation({
    mutationFn: (lang: Locale) => generate({ workspaceId, cwd, clientLocale, techId: tech.id, lang, force: false }),
    onSuccess() {
            ...
            {doc.sections.map((section, index) => {
              ...
              return (
                <Card key={`${section.heading}-${index}`} theme={theme}>
                  ...
                  <Text
                    selectable
                    style={{ color: unsourced ? theme.colors.foregroundMuted : theme.colors.foreground, lineHeight: 21 }}
                  >
                    {section.body}
                  </Text>
...
function QuizBox({ tech, workspaceId, cwd, clientLocale, theme, t, codeQuizAllowed, onChanged }: {
  tech: Technology;
  workspaceId: string;
  cwd: string;
  clientLocale: string | undefined;
  ...
  const question = useQuery({
    ...
    queryFn: () => nextQuiz({ workspaceId, cwd, clientLocale, techId: tech.id }),
    enabled: false,
    retry: 0,
  });
  const submit = useMutation({
    mutationFn: () => answerQuiz({ workspaceId, cwd, clientLocale, questionId: question.data!.id, answer }),
    onSuccess(value) {
      setResult({
        passed: value.passed,
...
function CommitsView({ data, theme, t }: { data: Dashboard; theme: PluginTheme; t: Translator }) {
  ...
  const agentCount = data.commits.filter((commit) => commit.authorship === "agent" || commit.authorship === "mixed").length;
                ...
                left={
                  <View style={{ flexDirection: "row", gap: 8, alignItems: "flex-start" }}>
                    ...
                    <View style={{ flex: 1, gap: 2 }}>
                      ...
                      <Mono
                        text={`${commit.sha.slice(0, 8)} · +${commit.insertions}/−${commit.deletions} · ${commit.filesChanged} · ${
                          relativeTime(t, commit.authoredAt, now)
                        }`}
                        theme={theme}
                      />
                    </View>
...
function SettingsView({ data, settings, theme, t, workspaceId, cwd, clientLocale, onLocaleChanged }: {
  data: Dashboard;
  ...
  const privacy = useMutation({
    mutationFn: (value: "public" | "private" | "airgapped") => setPrivacy({ workspaceId, cwd, clientLocale, privacy: value }),
...
export function RumenWorkspacePanel({ theme, host, layout, workspaceId }: PluginWorkspacePanelProps) {
  ...
  const scan = useMutation({
    mutationFn: () => scanWorkspace({ workspaceId, cwd, clientLocale }),
    onSuccess(value) {
  ...
  const classify = useMutation({
    mutationFn: () => classifyPending({ workspaceId, cwd, clientLocale }),
    onSuccess(value) {
  ...
  const evidence = useMutation({
    mutationFn: (node: KnowledgeNode) =>
      addEvidence({ workspaceId, cwd, clientLocale, nodeGroupId: node.groupId, kind: "wiki_read" }),
    onSuccess() {
  ...
  return (
    <View style={styles.screen}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {data && !data.generation.available && tab !== "settings"
          ? (
            <Card theme={theme}>
            ...
            ? (
              <View style={{ gap: 16 }}>
                ...
                <QuizBox
                  ...
                  theme={theme}
                  t={t}
                  codeQuizAllowed={data.generation.codeQuizAllowed}
                  onChanged={() => void dashboard.refetch()}
                />
              </View>
            )
            : (
        ...
        {data && tab === "commits" ? <CommitsView data={data} theme={theme} t={t} /> : null}
